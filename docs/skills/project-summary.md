...
## 2026-06-05 Runtime Note

Runtime env examples are now split into neutral, local QA, staging, and production templates. Reusable defaults use `demo`; `kasta` is local seed/demo or company-only context. OpenAI remains opt-in through `AI_PROVIDER=openai`, with a manual real-key smoke helper and safe-answer prompt baseline documented under `docs/runtime/`.

## 2026-06-05 Tenant AI Profile Note

Tenants can now have configurable AI profile basics: assistant identity, company display name, tone/business context, welcome/fallback/handoff messages, internal prompt guidance, and widget-safe branding basics. Admin editing is protected through the admin-web proxy; the customer widget receives only public display-safe fields.

## 2026-06-05 Persistent Human Support Note

Human handoff now persists as `PENDING_HUMAN` until the customer or an admin/agent explicitly ends it. Customer messages during human mode are saved for the agent without AI provider calls; agent replies do not automatically resume AI.
...
- 人工 handoff：客户请求人工支持后，conversation 进入持续的 `PENDING_HUMAN`；后台可分配支持用户、发送 agent reply、开启/结束人工模式，客户也可以自行结束人工支持来恢复 AI。
...
- Assistant reply defaults to deterministic/template flow; real OpenAI is implemented but opt-in and requires explicit env plus manual smoke verification.
